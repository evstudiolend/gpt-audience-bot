from fastapi import FastAPI, Request
from pydantic import BaseModel
from langchain.chat_models import ChatOpenAI
from langchain.chains import LLMChain
from langchain.prompts import PromptTemplate
import uvicorn
import os

app = FastAPI()

class InputData(BaseModel):
    question: str

# Загружаем ключ API из переменной окружения
openai_api_key = os.getenv("OPENAI_API_KEY")

# Создаём LLMChain
llm = ChatOpenAI(model_name="gpt-4", temperature=0, openai_api_key=openai_api_key)

prompt = f"""
Ты — маркетолог-аналитик. Проанализируй целевую аудиторию продукта, опираясь на описание ниже.

🔹 Входные данные:
\"\"\"{user_input}\"\"\"

🔹 Выполни глубокий структурированный анализ целевой аудитории по следующему алгоритму:

1. Сегментация по методике 5W:
- Who — кто потребитель (возраст, пол, профессия, отрасль, семейное положение и т.д.)
- What — что мы предлагаем (особенности продукта)
- Why — зачем покупают (выгоды, мотивация)
- When — когда нужен продукт
- Where — где найти и как привлечь эту аудиторию

2. Сегментация аудитории:

Географическая:
- Регион / область / город
- Климат
- Экономическая специализация
- Уровень урбанизации
- Количество жителей

Социально-демографическая:
- Пол
- Возраст
- Поколение
- Этап жизненного цикла
- Роль в домохозяйстве
- Наличие и возраст детей
- Образование
- Сфера деятельности и должность
- Уровень дохода

Поведенческая:
- Повод к покупке
- Частота покупки
- Эмоциональный эффект
- Степень готовности к покупке
- Каналы приобретения
- Этап клиентского пути

Психографическая:
- Ценности
- Стиль жизни
- Отношение к инновациям

B2B (если применимо):
- Отрасль
- Подкатегория компании
- Объем закупок
- География
- Роли закупочного комитета
- Уровень технологичности

3. Поведенческие и эмоциональные аспекты:
- Боли и проблемы
- Страхи и сомнения
- Типичные возражения
- Альтернативы
- Критерии выбора продукта и поставщика
- Ожидания и как их превзойти

4. Персонажи (персоны):
Сформулируй 1–2 типовых персонажа с деталями: кто они, чем занимаются, чего боятся, что хотят, почему купят, что их останавливает.

5. Итоги:
- Краткие выводы
- Рекомендации по коммуникации
- Каналы и точки контакта
- Как выделиться и завоевать доверие

Оформи ответ чётко, по пунктам, как в маркетинговом отчёте. Не выдумывай лишнего — анализируй.
"""

chain = LLMChain(llm=llm, prompt=prompt)

@app.post("/analyze")
async def analyze(data: InputData):
    response = chain.run(data.question)
    return {"result": response}

if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=80)
